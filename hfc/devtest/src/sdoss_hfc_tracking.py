...
"""
Python module to launch Sunspots tracking on SDO/HMI images for the HFC.
"""
__version__ = "1.1"
#__license__ = ""
...
STARTTIME=STARTTIME.strftime(INPUT_TFORMAT)
ENDTIME=ENDTIME.strftime(INPUT_TFORMAT)
...
# Fields to save
FIELDS=["ID_SUNSPOT","TRACK_ID","DATE_OBS",
...
def parse_configfile(configfile):
    ...
    try:
        ...
        LOG.error("Error parsing configuration file " + str(configfile) + ": " + str(why))
        return None
...
def get_filelist(starttime,endtime,
    ...
    if (data_directory.startswith("ftp")):
        ...
        # Years to cover
        startyear = starttime.year ; endyear=endtime.year
            ...
            try:
                ...
                for current_file in current_filelist:
                    ...
                    if (current_file.endswith("_feat.csv")):
                        ...
                        if (current_date < (starttime-dsec)): continue
                        if (current_date > (endtime+dsec)): continue
...
if (__name__ == "__main__"):
    ...
    parser.add_argument('-o','--output_directory',nargs='?',
                        ...
                        help="Pathname of the directory where output files are saved")
    ...
    LOG.info("Starting sdoss_hfc_tracking.py on "+HOSTNAME+" ("+TODAY.strftime(INPUT_TFORMAT)+")")    
    ...
    for i in range(nfile-max_files+1):
        ...
        current_trackset=[os.path.join(output_directory,
                                       os.path.basename(current_file).split("_feat.csv")[0]+"_track.csv")
        ...
        track_id, lvl_trust=tracking(feat_data_i['DATE_OBS'],
                                     feat_data_i['FEAT_X_PIX'],feat_data_i['FEAT_Y_PIX'],
                                     feat_data_i['FEAT_HG_LONG_DEG'],feat_data_i['FEAT_HG_LAT_DEG'],
                                     feat_data_i['FEAT_AREA_DEG2'],
                                     radius=radius,dt_max=dt_max,area_min=area_min,track_id=prev_track_id)
        ...
        for j,current_file in enumerate(current_fileset):
            ...
            for k,ci in enumerate(ind):
                output_data.append({'ID_SUNSPOT':k+1,
                                    ...
                                    'DATE_OBS':feat_data_i['DATE_OBS'][ci].strftime(INPUT_TFORMAT),
                                    'FEAT_X_PIX':feat_data_i['FEAT_X_PIX'][ci],
                                    'FEAT_Y_PIX':feat_data_i['FEAT_Y_PIX'][ci],
                                    'PHENOM':"", 'REF_FEAT':"", 
